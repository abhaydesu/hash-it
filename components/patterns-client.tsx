...
function PatternsInner({ patterns }: { patterns: PatternData[] }) {
  const searchParams = useSearchParams();
  const initialSelectedId = searchParams.get("selected");
  const [selectedPatternId, setSelectedPatternId] = useState<string | null>(
    initialSelectedId || (patterns.length > 0 ? patterns[0].id : null)
  );
...
export function PatternsClient({ patterns }: { patterns: PatternData[] }) {
  return (
    <Suspense>
      <PatternsInner patterns={patterns} />
    </Suspense>
  );
}
